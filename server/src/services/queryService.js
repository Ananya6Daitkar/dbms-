// Query Service - Registry of all predefined SQL queries

const queries = {
  // Simple Queries (s01-s10)
  s01: {
    id: 's01',
    title: 'All Customers',
    category: 'Simple',
...
WHERE item_id = (
  ...
  LIMIT 1
    ...
    explanation: 'Uses a subquery to find the item with the most ratings, then retrieves its name. The subquery groups ratings by item and counts them.',
  ...
  c02: {
    ...
    category: 'Complex',
    difficulty: 'Medium',
    sql: `SELECT r.restaurant_name, AVG(m.price) as avg_price
FROM Restaurant r
JOIN Menu_Item m ON r.restaurant_id = m.restaurant_id
GROUP BY r.restaurant_name
HAVING AVG(m.price) > 300`,
    entities: ['Restaurant', 'Menu_Item'],
    explanation: 'Joins restaurants with their menu items, groups by restaurant, calculates average price, and filters using HAVING clause for averages above 300.',
...
WHERE NOT EXISTS (
  ...
  WHERE o.customer_id = c.customer_id
    ...
    explanation: 'Uses NOT EXISTS to find customers with no matching orders. The subquery checks for order existence for each customer.',
  ...
  c04: {
    ...
    category: 'Complex',
    difficulty: 'Hard',
    sql: `SELECT restaurant_id, item_name, price
FROM Menu_Item m1
WHERE price = (
  SELECT MAX(price) 
  FROM Menu_Item m2 
  WHERE m1.restaurant_id = m2.restaurant_id
)`,
    entities: ['Menu_Item'],
    explanation: 'Correlated subquery that finds the maximum price for each restaurant, then returns items matching that price.',
  ...
  c05: {
    ...
    category: 'Complex',
    difficulty: 'Medium',
    sql: `SELECT c.first_name, SUM(p.amount)
FROM Customer c
JOIN Orders o ON c.customer_id=o.customer_id
JOIN Payment p ON o.order_no=p.order_no
GROUP BY c.first_name`,
    entities: ['Customer', 'Orders', 'Payment'],
    explanation: 'Joins three tables (Customer, Orders, Payment) to calculate total spending per customer using SUM and GROUP BY.',
...
export function getQueriesByCategory(category) {
  return Object.values(queries).filter(q => q.category === category);
}

// Get queries by difficulty
export function getQueriesByDifficulty(difficulty) {
  return Object.values(queries).filter(q => q.difficulty === difficulty);
}
